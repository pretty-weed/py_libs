...
class ForceableFailingAction(Action):
    ...
    def __init__(self, option_strings, dest, nargs=None, **kwargs):
        if self.FORCE_FLAG not in self._force_parsers:
            ...
            force_parser.add_argument(self.FORCE_FLAG, dest=self.FORCE_DEST, action="store_true")
            self.__class__._force_parsers[self.FORCE_FLAG] = force_parser

        parsed, _unparsed = self._force_parsers[self.FORCE_FLAG].parse_known_args()
    ...
    def _do_check(self, parser, namespace, values, result, option_string=None) -> None:
        raise NotImplementedError()

    def __call__(self, parser, namespace, values, option_string=None):

        print('%r %r %r' % (namespace, values, option_string))
        results = [self._get_val(parser, namespace, value, option_string) for value in values]
        for result, value in zip(results, values):
            try:
                self._do_check(parser, namespace, value, result, option_string=option_string)
